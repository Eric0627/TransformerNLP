...
class Utilities:
    def __init__(self, tokenizer, model, model_type):
    ...
    def sanity_check(self, sentence, block_size):
        ...
        if self.model_type == "encoder": # encoder
            attn_maps = self.model.encoder.get_attention_maps(input_tensor)
        else: # decoder
            attn_maps = self.model.get_attention_maps(input_tensor)
        ...
        for j, attn_map in enumerate(attn_maps):
            ...
            ax.set_title(f"Attention Map {j + 1}")
            
        # Save the plot
        if self.model_type == "encoder": 
            plt.savefig(f"CLS_attention_map.png")
        else:
            plt.savefig(f"LM_attention_map.png")
